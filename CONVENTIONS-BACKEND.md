...
| ToolFactory | `src/tools/test_*.py` | fake ToolProvider factory | 없음 |
...
- `AsyncMock(spec=AgentService)` fixture → `app.dependency_overrides[get_agent_service] = lambda: mock`
- 응답 상태코드와 body만 검증
...
- LLM과 ToolFactory는 생성자로 주입한다. TaskAgent의 툴 생명주기는 async context manager fake로 검증한다.
...
- 포맷: Ruff. 설정은 `backend/pyproject.toml`의 `[tool.ruff*]` 섹션에 둔다.
