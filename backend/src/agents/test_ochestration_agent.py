...
async def test_orchestration_agent_에이전트에게_프롬프트를_전달한다(mock_create_agent, mock_agent, mock_llm):
  ...
  sut = OrchestrationAgent(llm=mock_llm)
...
@patch("agents.orchestration_agent.create_agent")
async def test_orchestration_agent_일을_위임할_타겟_에이전트를_찾는다(mock_create_agent, mock_agent, mock_llm):
  mock_create_agent.return_value = mock_agent
  mock_agent.ainvoke.return_value = {
    "structured_response": OrchestrationAgentMessage(
      result=TargetAgent(name="검색 에이전트", system_prompt="웹 검색 담당"), is_applicable=True, reason="할일 관련 요청"
  ...
  sut = OrchestrationAgent(mock_llm)
...
@patch("agents.orchestration_agent.create_agent")
async def test_orchestration_agent_일을_위임할_타겟_에이전트를_찾지_못하면_None을_반환한다(mock_create_agent, mock_agent, mock_llm):
  mock_create_agent.return_value = mock_agent
  mock_agent.ainvoke.return_value = {
