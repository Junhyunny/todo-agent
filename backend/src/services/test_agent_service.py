...
from models.agent_api_schema import AgentRequest
...
async def test_create_agent_레포지토리_create_함수를_호출한다(mock_agent_repository: AsyncMock):
  ...
  mock_agent_repository.create.return_value = AgentModel(id=str(expected_id), name="에이전트1", system_prompt="프롬프트1")
...
async def test_get_all_agents_레포지토리를_통해_에이전트_리스트를_조회한다(mock_agent_repository: AsyncMock):
  ...
  mock_agent_repository.get_all.return_value = [
    AgentModel(id=str(expected_id_1), name="에이전트1", system_prompt="프롬프트1"),
    AgentModel(id=str(expected_id_2), name="에이전트2", system_prompt="프롬프트2"),
  ]
  sut = AgentService(agent_repository=mock_agent_repository)
...
async def test_update_agent_레포지토리_update_함수를_호출한다(mock_agent_repository: AsyncMock):
  expected_id = uuid.uuid4()
  mock_agent_repository.update.return_value = AgentModel(id=str(expected_id), name="에이전트1", system_prompt="프롬프트1")
  sut = AgentService(agent_repository=mock_agent_repository)
