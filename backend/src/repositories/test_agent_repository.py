...
async def test_create_에이전트_정보를_저장할_수_있다(setup_test_db: AsyncSession):
  ...
  agent = AgentModel(name="테스트 에이전트", system_prompt="테스트 프롬프트")
...
async def test_get_all_에이전트_리스트를_조회할_수_있다(setup_test_db: AsyncSession):
  ...
  session.add(AgentModel(id=expected_id_1, name="에이전트 1", system_prompt="프롬프트 1"))
  session.add(AgentModel(id=expected_id_2, name="에이전트 2", system_prompt="프롬프트 2"))
...
async def test_update_에이전트_정보를_업데이트할_수_있다(setup_test_db: AsyncSession):
  ...
  session = setup_test_db
  session.add(AgentModel(id=str(expected_id), name="업데이트 전 에이전트", system_prompt="업데이트 전 프롬프트"))

  sut = AgentRepository(session=setup_test_db)

  await sut.update(agent_id=expected_id, model=AgentModel(name="업데이트 후 에이전트", system_prompt="업데이트 후 프롬프트"))

  query = select(AgentModel).where(AgentModel.id == str(expected_id))
  result = await session.execute(query)
  updated_agent: AgentModel | None = result.scalar_one_or_none()
...
async def test_delete_에이전트_정보를_삭제할_수_있다(setup_test_db: AsyncSession):
  expected_id = uuid.uuid4()
  session = setup_test_db
  session.add(AgentModel(id=str(expected_id), name="삭제 전 에이전트", system_prompt="삭제 전 프롬프트"))
