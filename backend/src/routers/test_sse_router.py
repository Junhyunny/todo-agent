...
async def test_GET_todos_events_SSE_매니저를_구독한다(mock_async_client: AsyncClient, mock_sse_manager: MagicMock) -> None:
  ...
  mock_sse_manager.subscribe.assert_called_once_with("TODO_ASSIGN_CHANNEL_todo-1")


async def test_GET_todos_events_assigned_이벤트를_스트리밍한다(mock_async_client: AsyncClient) -> None:
  ...
  assert event["type"] == "assigned"
  assert event["agent_name"] == "검색 에이전트"


async def test_GET_todos_events_assigned_이벤트_후_구독을_해제한다(mock_async_client: AsyncClient, mock_sse_manager: MagicMock) -> None:
  await mock_async_client.get("/api/todos/todo-1/events")

  mock_sse_manager.unsubscribe.assert_called_once_with("TODO_ASSIGN_CHANNEL_todo-1")
