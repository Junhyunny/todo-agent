...
먼저 프로젝트 루트의 `ARCHITECTURE.md`가 있으면 읽습니다.

파일이 없으면 현재 프로젝트의 빌드 파일, 소스 루트, 테스트 루트를 직접 분석해 필요한 스택 정보를 추론합니다.

`ARCHITECTURE.md`가 있으면 그 `## 스택 요약`을 기준으로 삼고, 없으면 프로젝트 디렉토리 구조를 기준으로 컨벤션 생성 범위를 정합니다.

### Step 2: 기존 코드 스캔 (기존 프로젝트)

`ARCHITECTURE.md`에 기록된 각 스택 영역에서 독립적으로 스캔합니다.
...
`ARCHITECTURE.md`에 기록된 각 스택에 대해 위 파일 구조 템플릿의 **모든 섹션**을 채웁니다.
