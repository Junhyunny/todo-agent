...
3. 선택이 결정되면:
   ...
   - 필요하면 현재 프로젝트를 다시 스캔해 `.agents/tech-stack.md`를 갱신합니다
...
| `update stack` | 현재 프로젝트를 다시 스캔해 `.agents/tech-stack.md`를 갱신 |
| `update conventions` | 최근 변경 파일과 프로젝트 상태를 다시 분석해 `.agents/coding-conventions.md`를 갱신 |
...
| `references/coding-conventions-template.md` | 컨벤션 파일 생성/갱신 시 — 파일 구조 및 생성 규칙 |
