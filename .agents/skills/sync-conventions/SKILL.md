...
이 스킬의 목적은 **프로젝트 전체를 무조건 다시 스캔하는 것**이 아니라, 다음 신호를 바탕으로 `CONVENTIONS.md`를 생성 또는 갱신하는 것이다:
...
- `CONVENTIONS.md`가 **있으면**, 이 파일을 현재 기준점으로 사용한다
- `CONVENTIONS.md`가 **없으면**, 새로 생성하기 위해서만 프로젝트 코드를 직접 읽고 스캔한다
- `ARCHITECTURE.md`가 있으면 참고할 수는 있지만, 이 스킬의 실행 조건이 아니며 없어도 계속 진행한다
...
- `CONVENTIONS.md`가 이미 있으면 **"현재 컨벤션 파일이 최신으로 보입니다."**처럼 짧게 표시하고 종료한다
...
- 동일 영역의 대표 파일 1~2개
- `ARCHITECTURE.md`가 이미 있으면 보조 힌트로만 읽을 수 있다

### B. `CONVENTIONS.md`가 없는 경우
...
변경된 파일과 기존 `CONVENTIONS.md`를 비교해 다음 유형의 드리프트를 찾는다:
...
→ "yes" — 현재 패턴을 `CONVENTIONS.md`에 반영
